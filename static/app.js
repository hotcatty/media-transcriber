...
function buildCopyForAi(task, transcript) {
  const lines = [
    ...
    "2. 重点与金句：重点 3–8 条，每条一句话只写一个观点。金句从正文逐字引用，用短定位说明出处（如「谈到 XX 时」），不要编段落号。",
    "3. 分段全文：一段一个话题或一轮发言，段间空一行。除上述校对外不增删内容。直接写在回复里；若支持生成 text 文件，文件必须保留换行，禁止连成一块。能一次写完就不要拆；只有会超出回复上限时才分次，每次开头标明「第 X 部分／共 Y 部分」。",
    "4. 若正文主要是英文：总结、重点与金句用中文；分段全文仍用英文；最后再附同样分段的中文译文。",
    "",
  ];
